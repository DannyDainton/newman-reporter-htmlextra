 ...
 * @param {String=} options.skipFolders - Optional flag to skip certain requests in the final report.
...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    // Helper for checking if element is in list
    handlebars.registerHelper('containsFolder', function (item) {
        return _.isString(options.skipFolders) && options.skipFolders.split(',').includes(item)
    });
