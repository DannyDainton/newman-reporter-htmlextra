...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    if (collectionRunOptions.reporter.json || collectionRunOptions.reporter.progress || collectionRunOptions.reporter.cli) {
        newman.on('start', function (err, o) {
            if (err) { return; }
            console.log(`Using ${chalk.green('htmlextra')} version ${chalk.green(version)}`)
        });
    }
    if (!collectionRunOptions.reporter.progress && !collectionRunOptions.reporter.cli && !collectionRunOptions.reporter.json) {
        ...
        newman.on('done', function () {
            bar.stop();
            console.log(`Created the ${chalk.green('htmlextra')} report in this location: ${collectionRunOptions.reporter.htmlextra.export ? chalk.green(collectionRunOptions.reporter.htmlextra.export) : chalk.green('/newman')}`);
        });
    }
