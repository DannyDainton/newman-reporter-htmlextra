...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    handlebars.registerHelper('maskKeyInResponse', function (responseBody) {
        let parsedResponse,
            maskedResponse,
            keysToMask = options.maskResponseKey;
        if (keysToMask.length < 0) {
            return responseBody;
        }
        parsedResponse = JSON.parse(responseBody);
        maskedResponse = JSON.stringify(parsedResponse, function (key, val) {
    ...
    newman.on('beforeDone', function () {
        ...
        this.exports.push({
            ...
            content: compiler({
                ...
                maskResponseKey: options.maskResponseKey || [],
