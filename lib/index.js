...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    if (collectionRunOptions.reporter.progress || collectionRunOptions.reporter.cli) {
    ...
    if (!collectionRunOptions.reporter.progress && !collectionRunOptions.reporter.cli) {
        ...
        newman.on('done', function () {
            bar.stop();
            console.log(`File export location: ${collectionRunOptions.reporter.htmlextra.export ? chalk.green(collectionRunOptions.reporter.htmlextra.export) : chalk.green('/newman')}`);
        });
    }
